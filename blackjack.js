...
var dealerScoreNode = element.getElementById('dealer-number')
// select the DOM node which has id="player-number"
var playerScoreNode = element.getElementById("player-number")

// select the DOM node which has id="dealer-cards"
var dealerCardsNode = element.getElementById("dealer-cards")

// select the DOM node which has id="player-cards"
var playerCardsNode = element.getElementById("player-cards")

// selec the DOM node which has id="announcement"
var announcementNode = element.getElementById("announcement")

// selec the DOM node which has id=new-game"
var newDeckNode = element.getElementById("new-game")

// selec the DOM node which has id="next-hand"
var nextHandNode = element.getElementById("next-hand")

// selec the DOM node which has id=""hit-me""
var hitMeNode = element.getElementById("hit-me")

// selec the DOM node which has id="stay"
var stayNode = element.getElementById("stay")
  ...
  */
  resetPlayingArea () {
    ...
    .then(response => { 
      ...
      playerCards.push(response[0], response[1])
      dealerCards.push(response[2], response[3])
      
      dealerScore = "?"; 
      dealerCardsNode.textContent = dealerScore
      ...
      if (card === 0) {
        ...
        dealerCardsNode.appendChild(cardElement);
      })
    playerCards.forEach(card=> {
     var cardElement = document.createElement('img');
     cardElement.src = card.image;
     playerCardsNode.appendChild(cardElement);
    })
    ...
    playerCardsNode.textContent = playerScore
...
function hitMe(target) {
  ...
  */
if (roundLost || roundWon || roundTied === true) {return}
fetch(`https://deckofcardsapi.com/api/deck/new/draw/?count=2`)
.then(response => response.json())
.then(response => { 
...
function dealerPlays() {
  ...
  */
  ...
  if(roundLost || roundWon || roundTied === true){
    return;
  }
